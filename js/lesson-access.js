...
const LessonAccess = {
    ...
    async check(lessonId) {
        ...
        const courseId  = localStorage.getItem('studentCourseId') || localStorage.getItem('studentCourse'); // Try both

        // 1. Find potential rules for this lesson
        const lessonRules = rules.filter(r => r.lesson_id === lessonId);
        if (lessonRules.length === 0) return { allowed: true };

        // 2. Specific Student Match
        if (studentId) {
            const studentRule = lessonRules.find(r => r.student_id === studentId);
            if (studentRule) return this._evaluateRule(studentRule);
        }

        // 3. Course Match
        if (courseId) {
            const courseRule = lessonRules.find(r => r.course_id === courseId);
            if (courseRule) return this._evaluateRule(courseRule);
        }

        // 4. Global Rule (if any, e.g. course_id and student_id are null)
        const globalRule = lessonRules.find(r => !r.course_id && !r.student_id);
        if (globalRule) return this._evaluateRule(globalRule);
